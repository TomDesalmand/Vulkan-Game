...
namespace vulkan {
    ...
    Pipeline::Pipeline(Device &device, const std::string &vertFilepath, const std::string &fragFilepath, const PipelineConfigurationInformation &configurationInformation) : _device{device} {
        if (configurationInformation.pipelineLayout == VK_NULL_HANDLE) {
            ERROR("Cannot create graphics pipeline:: no pipelineLayout in configurationInformation.");
            throw std::runtime_error("Cannot create graphics pipeline:: no pipelineLayout in configurationInformation.");
        }
        if (configurationInformation.renderPass == VK_NULL_HANDLE) {
            ERROR("Cannot create graphics pipeline:: no renderPass in configurationInformation.");
            throw std::runtime_error("Cannot create graphics pipeline:: no renderPass in configurationInformation.");
        }
        ...
        pipelineInformation.stageCount = 2; // Count of how many programmable stages the pipeline will use. (number of shaders) //
