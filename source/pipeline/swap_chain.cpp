...
namespace vulkan {
    ...
    SwapChain::SwapChain(Device &deviceRef, VkExtent2D extent, size_t maxFramesInFlight) : _device{deviceRef}, _windowExtent{extent}, _maxFramesInFlight{maxFramesInFlight} {
        init();
    }

    SwapChain::SwapChain(Device &deviceRef, VkExtent2D extent, std::shared_ptr<SwapChain> previous, size_t maxFramesInFlight) : _device{deviceRef}, _windowExtent{extent}, _oldSwapChain{previous}, _maxFramesInFlight{maxFramesInFlight} {
        init();
    ...
    void SwapChain::createSyncObjects() {
        ...
        if (getImageCount() > 0 && _maxFramesInFlight > getImageCount()) {
            _maxFramesInFlight = getImageCount();
        }
        ...
        for (size_t i = 0; i < _maxFramesInFlight; i++) {
            if (vkCreateSemaphore(_device.getDevice(), &semaphoreInformation, nullptr, &_imageAvailableSemaphores[i]) != VK_SUCCESS ||
                vkCreateFence(_device.getDevice(), &fenceInformation, nullptr, &_inFlightFences[i]) != VK_SUCCESS) {
                throw std::runtime_error("Failed to create synchronization objects for a frame.");
            }
        }

        for (size_t i = 0; i < getImageCount(); i++) {
            if (vkCreateSemaphore(_device.getDevice(), &semaphoreInformation, nullptr, &_renderFinishedSemaphores[i]) != VK_SUCCESS) {
